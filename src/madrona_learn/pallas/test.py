...
import madrona_learn.pallas.monkeypatch
...
class FusedLayerNormTest(PallasTest):
  ...
  @parameterized.parameters(*[
    (1, 384, 192, jnp.float32),
    (2, 384, 192, jnp.float32),
    #(1, 327680, 256, jnp.float32),
    #(1, 327680, 256, jnp.float32),
    #(1, 81920, 256, jnp.float32),
    (81920, 1, 256, jnp.float32),
  ])
  def test_fused_layernorm_fwd(self, batch_size, seq_len, embed_dim, dtype):
    if plgpu.get_compute_capability(0) < 70:
      raise unittest.SkipTest(
          "Fused layernorm only works on GPUs with capability >= sm70")

    def test(rnd):
        k1, k2, k3 = random.split(rnd, 3)
        x = random.normal(k1, (batch_size, seq_len, embed_dim), dtype=dtype)
    ...
    # Double vmap:
    #test = jax.jit(jax.vmap(jax.vmap(test)))
    #o, o_ref = test(jax.vmap(random.split, in_axes=(0, None))(random.split(rnd, 1), 2))

    # Single vmap
    test = jax.jit(jax.vmap(test))
    o, o_ref = test(random.split(rnd, 8))

    # no vmap:
    #o, o_ref = jax.jit(test)(rnd)
  ...
  def test_fused_layernorm_bwd(self, batch_size, seq_len, embed_dim, dtype):
    if plgpu.get_compute_capability(0) < 70:
      raise unittest.SkipTest(
          "Fused layernorm only works on GPUs with capability >= sm70")
    ...
    def test(rnd):
        ...
        return dx, dw, db, dx_ref, dw_ref, db_ref
    ...
    dx, dw, db, dx_ref, dw_ref, db_ref = test(random.split(rnd, 8))
