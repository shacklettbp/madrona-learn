...
def _process_grid_to_3d_grid(builder, grid_mapping: GridMapping):
  if len(grid_mapping.grid) == 0:
    return grid_mapping.grid, []

  total_axis_size = np.prod(grid_mapping.grid)
  new_grid = (total_axis_size,)

  def make_subidx(cur_idx, s):
    return (
        cur_idx.__floordiv__(s, _builder=builder),
        cur_idx.__mod__(s, _builder=builder),
    )

  grid0 = tl.program_id(0, _builder=builder)
  out_indices = [0] * len(grid_mapping.grid)

  # Preserve existing ordering for non-mapped dims
  for i, s in enumerate(grid_mapping.grid):
    if i in grid_mapping.mapped_dims:
      continue

    grid0, out_indices[i] = make_subidx(grid0, s)

  # For mapped dims, iterate the inner mapped dims to the outer dims. This 
  # follows the pallas_call batching rule that prepends the vmapped dimension.
  for dim in reversed(grid_mapping.mapped_dims):
    s = grid_mapping.grid[dim]
    grid0, out_indices[dim] = make_subidx(grid0, s)
